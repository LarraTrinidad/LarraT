...
#
selected_columns <- test %>% select(time_s, mean_bleached_1, mean_bleached_2, mean_bleached_3, mean_bleached_4, mean_bleached_5, mean_bleached_6, mean_bleached_7, mean_unbleached_1, mean_unbleached_2, mean_unbleached_3, mean_unbleached_4)
...
start_bleached <- c(start_bleached_1, start_bleached_2, start_bleached_3, start_bleached_4, start_bleached_5, start_bleached_6, start_bleached_7)
...
post_bleach <- c(post_bleach_1, post_bleach_2, post_bleach_3, post_bleach_4, post_bleach_5, post_bleach_6, post_bleach_7)
...
bleached_regions <- test %>% select(mean_bleached_1, mean_bleached_2, mean_bleached_3, mean_bleached_4, mean_bleached_5, mean_bleached_6, mean_bleached_7)
...
normalised_for_acquisition_bleaching_only <- normalised_for_acquisition_bleaching %>% select(time_s, normalised_acquisition_1, normalised_acquisition_2, normalised_acquisition_3, normalised_acquisition_4, normalised_acquisition_5, normalised_acquisition_6, normalised_acquisition_7)
...
normalised_between_0_and_1_only <- normalised_between_0_and_1 %>% select(time_s, Normalised_Region_1, Normalised_Region_2, Normalised_Region_3, Normalised_Region_4, Normalised_Region_5, Normalised_Region_6, Normalised_Region_7)
#normalised_between_0_and_1_only

normalisation <- normalised_between_0_and_1 %>% select(Normalised_Region_1, Normalised_Region_2, Normalised_Region_3, Normalised_Region_4, Normalised_Region_5, Normalised_Region_6, Normalised_Region_7)
normalised_average_sd <- normalisation %>%
  mutate(Normalised_Average = rowMeans(normalisation, na.rm=TRUE), Standard_Deviation = rowSds(as.matrix(normalisation[,c(1,2,3,4,5,6,7)]), na.rm=TRUE))
...
raw_bleached <- data.frame(t=time, y=mb1, Region = "1") %>% rbind(., data.frame(t=time, y=mb2, Region = "2")) %>% rbind(., data.frame(t=time, y=mb3, Region = "3")) %>% rbind(., data.frame(t=time, y=mb4, Region = "4")) %>% rbind(., data.frame(t=time, y=mb5, Region = "5")) %>% rbind(., data.frame(t=time, y=mb6, Region = "6")) %>% rbind(., data.frame(t=time, y=mb7, Region = "7"))
...
summary_normalised <- cbind(normalised_between_0_and_1 %>% select(time_s, normalised_acquisition_1, normalised_acquisition_2, normalised_acquisition_3, normalised_acquisition_4, normalised_acquisition_5, normalised_acquisition_6, normalised_acquisition_7, Normalised_Region_1, Normalised_Region_2, Normalised_Region_3, Normalised_Region_4, Normalised_Region_5, Normalised_Region_6, Normalised_Region_7), normalised_means_sd)
...
normalised_plot <- data.frame(t=time_s, y=nr1, Region = "1") %>% rbind(., data.frame(t=time_s, y=nr2, Region = "2")) %>% rbind(., data.frame(t=time_s, y=nr3, Region = "3")) %>% rbind(., data.frame(t=time_s, y=nr4, Region = "4")) %>% rbind(., data.frame(t=time_s, y=nr5, Region = "5")) %>% rbind(., data.frame(t=time_s, y=nr6, Region = "6")) %>% rbind(., data.frame(t=time_s, y=nr7, Region = "7")) %>% rbind(., data.frame(t=time_s, y=ave, Region = "Average"))
...
data_plot <- summary_normalised %>% select(time_s, Normalised_Region_1, Normalised_Region_2, Normalised_Region_3, Normalised_Region_4, Normalised_Region_5, Normalised_Region_6, Normalised_Region_7, Normalised_Average) %>% slice(4:36)
data_plot
...
rbind(., data.frame(t=t, y=y4, Region = '4')) %>%
  rbind(., data.frame(t=t, y=y5, Region = '5')) %>%
   ...
   rbind(., data.frame(t=t, y=y7, Region = '7')) %>% mutate(y = replace(y, y == "NA", NA)) %>% drop_na()
...
B <- Average %>% nest(-Region) %>%
  mutate(fit_B = map(data, ~nls(y ~ yf * percentage_fast * 0.01 * (1- exp(-alpha_fast * t)) + yf * (100 - percentage_fast) * 0.01 * (1-exp(-alpha_slow * t)), start = list(yf = yf_initial, percentage_fast = percentage_fast_initial, alpha_fast = alpha_initial_fast, alpha_slow = alpha_initial_slow), control = list(maxiter = 500, tol=5e-10, printEval = TRUE, warnOnly = TRUE, minFactor = 1/4096), algorithm = "port",  lower=c(0,0,0,0), upper=c(100,100,100,100), data=Average)), tidied_B = map(fit_B, tidy), ) %>% unnest(tidied_B) %>% select(Region, term, estimate) %>% spread(term, estimate)
         
B

B_mutated <- B %>% mutate("Time Constant Fast" = 1/alpha_fast,"Time Constant Slow" = 1/alpha_slow, "Half-Life Fast" = log(2, exp(1))/alpha_fast, "Half-Life Slow" = log(2, exp(1))/alpha_slow)
B_mutated


B_mu_alpha_fast = as.numeric(B[1,2])
B_mu_alpha_slow = as.numeric(B[1,3])
B_mu_yf = as.numeric(B[1,5])
B_mu_percentage_fast = as.numeric(B[1,4])


C <- A %>% mutate("Time Constant Fast" = 1/alpha_fast, "Time Constant Slow" = 1/alpha_slow, "Half-Life Fast" = log(2, exp(1))/alpha_fast, "Half-Life Slow" = log(2, exp(1))/alpha_slow)
C

results_yf <- t.test(C$yf, mu = B_mu_yf)
results_yf
results_percentage_fast <- t.test(C$percentage_fast, mu = B_mu_percentage_fast)
results_percentage_fast
results_alpha_fast <- t.test(C$alpha_fast, mu = B_mu_alpha_fast)
results_alpha_fast
results_alpha_slow <- t.test(C$alpha_slow, mu = B_mu_alpha_slow)
results_alpha_slow
results_tau_fast <- t.test(C$"Time Constant Fast", mu = 1/B_mu_alpha_fast)
results_tau_fast
results_tau_slow <- t.test(C$"Time Constant Slow", mu = 1/B_mu_alpha_slow)
results_tau_slow
results_t_0.5_fast <- t.test(C$"Half-Life Fast", mu = log(2, exp(1))/B_mu_alpha_fast)
results_t_0.5_fast
results_t_0.5_slow <- t.test(C$"Half-Life Slow", mu = log(2, exp(1))/B_mu_alpha_slow)
results_t_0.5_slow


yf <- B[1,5]
percentage_fast <- B[1,4]
alpha_fast <- B[1,2]
alpha_slow <- B[1,3]


augmented_ave <- Average %>% nest(-Region) %>% mutate(fit = map(data, ~nls(y ~ yf * percentage_fast * 0.01 * (1- exp(-alpha_fast * t)) + yf * (100 - percentage_fast) * 0.01 * (1-exp(-alpha_slow * t)), start = list(yf = yf_initial, percentage_fast = percentage_fast_initial, alpha_fast = alpha_initial_fast, alpha_slow = alpha_initial_slow), control = list(maxiter = 500, tol=5e-10, printEval = TRUE, warnOnly = TRUE), algorithm = "port", lower=c(0,0,0,0), upper=c(100,100,100,100), data=Average)), augmented = map(fit, augment), ) %>% unnest (augmented)

augmented_ave
summary(augmented_ave)

qplot(t, y, data = augmented_ave) + geom_point(color="mediumseagreen") + geom_line(aes(y=.fitted), color="mediumseagreen") + ylab("Average Normalised Intensity") + xlab("Time(s)") + labs(title= "Two-phase exponential fit of average normalised intensity") + theme_minimal()

fit_ave <- nls(y ~ yf * percentage_fast * 0.01 * (1- exp(-alpha_fast * t)) + yf * (100 - percentage_fast) * 0.01 * (1-exp(-alpha_slow * t)),  data = Average, start = list(yf = yf_initial, percentage_fast = percentage_fast_initial, alpha_fast = alpha_initial_fast, alpha_slow = alpha_initial_slow), list(maxiter = 500, tol=5e-10, minFactor = 1/4096, printEval = TRUE, warnOnly = TRUE), algorithm = "port", lower=c(0,0,0,0), upper=c(100,100,100,100))
summary(fit_ave)
#fit_ave


qplot(t, y, data=augment(fit_ave)) + geom_point(color="mediumseagreen") + geom_line(aes(y=.fitted), color="mediumseagreen") + ylab("Average Normalised Intensity") + xlab("Time(s)") + labs(title= "Two-phase exponential fit of average normalised intensity") + theme_minimal()


print(paste("The y_max of the graph: ", yf, "or", yf*100, "% instability"))
print(paste("The percentage_fast of the graph: ", percentage_fast, "or", percentage_fast*100, "% instability"))

print(paste("The rate constant, α fast: ", alpha_fast))
print(paste("The rate constant, α slow: ", alpha_slow))

tau_fast <- 1/alpha_fast
print(paste("The time constant, τ fast: ", tau_fast))

tau_slow <- 1/alpha_slow
print(paste("The time constant, τ slow: ", tau_slow))

t_0.5_fast <- log(2, exp(1)) / alpha_fast
print(paste("The half-life, t_0.5_fast: ", t_0.5_fast))

t_0.5_slow <- log(2, exp(1)) / alpha_slow
print(paste("The half-life, t_0.5_slow: ", t_0.5_slow))

pre_bleach_intensities <- c(unbleached_first_three_1, unbleached_first_three_2, unbleached_first_three_3, unbleached_first_three_4, start_bleached_1, start_bleached_2, start_bleached_3, start_bleached_4, start_bleached_5, start_bleached_6, start_bleached_7)
...
values <- data.frame("Wing Membrane Distance" = Wing_Membrane_Distance, "Background Intensity Value" = background_intensity_value, "ymax" = as.numeric(yf), "percentage_fast" = as.numeric(percentage_fast), "Rate Constant Fast" = as.numeric(alpha_fast), "Rate Constant Slow" = as.numeric(alpha_slow), "Time Constant Fast" = as.numeric(tau_fast), "Time Constant Slow" = as.numeric(tau_slow), "Half-Life Fast" = as.numeric(t_0.5_fast), "Half-Life Slow" = as.numeric(t_0.5_slow), "Mean Intensity" = mean_pre_bleach_intensities_no_background, "y0, corrected mean intensity" = as.numeric(y0))
values
...
sink(paste("Two-phase", basename(name), "Results.csv", sep = " "))
cat('')
...
combined <- tibble("Name" = name, "Wing_Membrane_Distance" = Wing_Membrane_Distance, "Puncta_Mean_Intensity" = as.numeric(mean_pre_bleach_intensities_no_background), "ymax" = as.numeric(yf), "y0" = as.numeric(y0), "Percentage_fast" = as.numeric(percentage_fast), "Percent_ymax_unstable" = as.numeric(yf*100), "Unstable_Amount" = unstable_amount, "Stable_Amount" = stable_amount, "Rate Constant Fast" = as.numeric(alpha_fast), "Rate Constant Slow" = as.numeric(alpha_slow), "Time Constant Fast" = as.numeric(tau_fast), "Time Constant Slow" = as.numeric(tau_slow), "Half-Life Fast" = as.numeric(t_0.5_fast), "Half-Life Slow" = as.numeric(t_0.5_slow))
combined
write.table(combined, "Two-phase Combined Results.csv", sep = ",", col.names = !file.exists("Two-phase Combined Results.csv"), row.names = FALSE, append = T)
Z <- read_csv("Two-phase Combined Results.csv")
Table_Summary <- distinct(Z, Name, .keep_all=TRUE)
Table_Summary
write.table(Table_Summary, "Two-phase Combined Results.csv", sep = ",", col.names = TRUE, row.names = FALSE, append = F)
...
write.table(average_intensities_for_plot, "Two-phase Average Intensities for Plot.csv", sep = ",", col.names = !file.exists("Two-phase Average Intensities for Plot.csv"), row.names = FALSE, append = T)
U <- read_csv("Two-phase Average Intensities for Plot.csv")
Average_Summary <- distinct(U, .keep_all=TRUE)
write.table(Average_Summary, "Two-phase Average Intensities for Plot.csv", sep = ",", col.names = TRUE, row.names = FALSE, append = F)
